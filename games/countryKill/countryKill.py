# 写的什么玩意啊我草
import random
from static import namePure

# 玩家类
class Player:
    def __init__(self, name: str, gender: str, maxLife: int, cards: list, power: str, role: str=None):
        # 昵称
        self.name = name # 昵称
    ...
    def dying(self):
        ...
        if self.life < 1:
            ...
            if len(countryKill[1]) == 1:
                ...
                text += f"{self.name}的身份是=={self.role}==！\n"
                if self.role == "主公":
                    winners = "@ ".join(getRoles("反贼"))
                    gameOver()
                    text += f"反贼(@{winners} )获胜！🍾🍾🍾\n"
                elif not (getRoles("反贼") or getRoles("内奸")):
                    lord = "".join(getRoles("主公"))
                    winners = "@ ".join(getRoles("忠臣"))
                    gameOver()
                    text += f"主公(@{lord} )与忠臣(@{winners} )获胜！🍾🍾🍾\n"

            if countryKill[0]:
                for card in self.cards:
                    cardList.appTrash(card)
                for card in (self.equipments + self.delays):
                    cardList.appTrash(card)
                del countryKill[2][self.name]
                if countryKill[3]["player"] == self.name:
                    countryKill[3]["die"] = True
                else:
                    killerObj: Player = countryKill[2][countryKill[3]["player"]]
                    if killerObj.role == "主公":
                        if self.role == "反贼":
                            text += f"{killerObj.name}摸3张牌。\n"
                            killerObj.draw(3)
                        elif self.role == "忠臣":
                            text += f"{killerObj.name}失去所有手牌。\n"
                            for card in killerObj.cards:
                                cardList.appTrash(card)
        return text
    ...
    def kill(self, target: str, card: "Card"):
        ...
        if not killObj["weaponed"]:
            if self.isEquipped("三尖两刃刀"):
                ...
                countryKill[3]["targets"].append(target)
    ...
    def formatAll(self) -> str:
        text = ""
        for k in countryKill[1]:
            v = countryKill[2][k]
            if v.role == "主公":
                text += f"### {k}({v.gender}，{v.power}，==主公==)(与您距离{self.distanceTo(k)})\n{v.formatCards()}\n"
            else:
...
def arrowBarbarian(sender: str, arrow: bool=False) -> str:
    ...
    return text
# 是否有身份
def getRoles(role: str) -> list[str]:
    result = []
    for player in countryKill[1]:
        playerObj = countryKill[2][player]
        if playerObj.role == role:
...
def deal(context):
    # 身份牌
    roles = []
    counts = ROLES.get(len(countryKill[1]))
    if counts:
    ...
    for i in countryKill[1]:
        ...
        if role == "主公":
            ...
            maxLife = 4
        countryKill[2][i] = Player(i, gender, maxLife, cards, power, role)
        if role:
            context.appText(f"您的性别是{gender}，势力是{power}，身份是=={role}==。以下是您的牌\n{countryKill[2][i].formatHand()}", "whisper", to=i)
        else:
...
def play(context, sender, msg):
    ...
    if command == "check":
        ...
        if senderObj.role:
            text += f", 身份：{senderObj.role}"
        return context.appText(text + f"\n{senderObj.formatCards()}\n#### 手牌:\n{senderObj.formatHand()}", "whisper")
    elif command == "help":
        card = msg[5:]
        if card in USAGE:
            text = USAGE[card]
            ...
            if sender == turn["temp"]:
                ...
                if turnCmd == "雌雄双股剑":
                    ...
                    if cardName and cardName[-1] == "杀":
                        turn["wait"] = False
                        text += checkKill(senderObj, cardObj)
                        turn["temp"] = None
                    elif command == ".":
                        turn["targets"] = []
                        turn["wait"] = False
                    ...
                    if command == ".":
                        ...
                        for i in array[:2]:
                            ...
                            discards.append(card.name)
                            cardList.appTrash(card)
                        turn["wait"] = False
                        text += f"{sender}弃置了{'、'.join(discards)}，对{targetObj.name}造成伤害！\n"
                    ...
                    if command == ".":
                        ...
                        for cmd in array:
                            if cmd == "手":
                                ...
                                equipment = targetObj.equipments[int(cmd)-1]
                                text += targetObj.unequip(equipment, True)
                        turn["wait"] = False
                elif turnCmd == "三尖两刃刀":
                    if command == ".":
        ...
        # 主动出牌
            ...
            if command == ".":
                ...
                if turn["legal"] and turn["cmd"] != "借刀杀人":
                    turn["cmd"] = cardName
        else:
            return
...
def goTurn(context):
    ...
    if playerObj.life < 1 and playerObj.name in countryKill[1]:
        getNext(True)
        return goTurn(context)
