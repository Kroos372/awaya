...
class OddEven:
    ...
    def initOE(self):
        ...
        self.last = 0
    def bet(self, odds: bool, nick: str, trip: str, money: int) -> str:
        if money < self.last:
        ...
        if odds:
            self.odds["users"].append({"nick": nick, "trip": trip, "money": money})
            self.odds["total"] += money
        else:
            self.evens["users"].append({"nick": nick, "trip": trip, "money": money})
...
def main(context: Context, sender: str, msg: str, trip: str):
    if msg == "help":
        ...
        try:
            ...
            if array[0] == "单":
                context.appText(oddeven.bet(True, sender, trip, money))
            elif array[0] == "双":
                context.appText(oddeven.bet(False, sender, trip, money))
            else:
                context.appText("参数错误！")
